...
from linuxsolo import dispatcher, spamfilters, LOGGER
from linuxsolo.modules.disable import DisableAbleCommandHandler
from linuxsolo.modules.helper_funcs.chat_status import user_admin_no_reply, user_admin

from linuxsolo.modules.sql import languages_sql as sql
...
for x in os.listdir('linuxsolo/modules/langs'):
	if os.path.isdir('linuxsolo/modules/langs/'+x):
		continue
	x = x.replace('.py', '')
	LOADED_LANGS_ID.append(x)
	imported_langs = importlib.import_module("linuxsolo.modules.langs." + x)
